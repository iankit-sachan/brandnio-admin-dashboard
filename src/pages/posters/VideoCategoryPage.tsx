...
import { videoCategoriesApi } from '../../services/admin-api'
...
const emptyForm: FormState = {
  name: '', icon_url: '', sort_order: 0, is_active: true,
}

export default function VideoCategoryPage() {
  const { addToast } = useToast()
  const { data, loading, create, update, remove } = useAdminCrud<VideoCategory>(videoCategoriesApi)
  const [modalOpen, setModalOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<VideoCategory | null>(null)
  const [form, setForm] = useState<FormState>(emptyForm)
  const [deleteItem, setDeleteItem] = useState<VideoCategory | null>(null)
  ...
  const openEdit = (item: VideoCategory) => {
    setEditingItem(item)
    setForm({
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingItem) {
        ...
        addToast('Video category updated successfully')
      } else {
        await create(form)
        addToast('Video category created successfully')
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      addToast('Video category deleted successfully')
      setDeleteItem(null)
    } catch {
  ...
  const toggleActive = useCallback(async (item: VideoCategory) => {
    try {
      const updated = await update(item.id, { is_active: !item.is_active } as any)
      if (updated) addToast(`Category "${item.name}" ${!item.is_active ? 'activated' : 'deactivated'}`)
    } catch {
  ...
  const columns: Column<VideoCategory>[] = [
    {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-brand-text">Video Categories</h1>
      ...
      <p className="text-sm text-brand-text-muted">
        Manage video template categories. Drag rows to reorder, toggle to show/hide.
      </p>
      ...
      <Modal isOpen={modalOpen} onClose={() => { setForm(emptyForm); setEditingItem(null); setModalOpen(false) }} title={editingItem ? 'Edit Video Category' : 'Add Video Category'} size="md">
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-brand-text-muted mb-1.5">Name</label>
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Intro Videos" className="w-full bg-brand-dark border border-brand-dark-border rounded-lg px-4 py-2.5 text-sm text-brand-text focus:outline-none focus:border-brand-gold/50" />
          </div>
          <div>
      ...
      <ConfirmDialog isOpen={!!deleteItem} onClose={() => setDeleteItem(null)} onConfirm={handleDelete} title="Delete Video Category" message={`Delete "${deleteItem?.name}"? This will remove the category and may affect associated video templates.`} confirmText="Delete" variant="danger" />
    </div>
  )
}
