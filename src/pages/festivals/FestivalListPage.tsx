...
import { mockFestivals } from '../../services/mock-data'
...
export default function FestivalListPage() {
  ...
  const [data, setData] = useState<Festival[]>([...mockFestivals])
  ...
  const handleSubmit = () => {
    ...
    if (editingItem) {
      setData(prev => prev.map(d => d.id === editingItem.id ? { ...d, banner_url: form.banner_url, icon_url: form.icon_url, name: form.name, slug: form.slug, description: form.description, date: form.date, is_active: form.is_active, is_upcoming: new Date(form.date) > new Date() } : d))
      addToast('Festival updated successfully')
    } else {
      const newItem: Festival = { id: Date.now(), name: form.name, slug: form.slug, description: form.description, date: form.date, banner_url: form.banner_url, icon_url: form.icon_url, is_active: form.is_active, is_upcoming: new Date(form.date) > new Date(), poster_count: 0, created_at: new Date().toISOString() }
  ...
  return (
    <div className="space-y-4">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingItem ? 'Edit Festival' : 'Add Festival'}>
        <div className="space-y-4">
